...
void Field::mousePressEvent(QGraphicsSceneMouseEvent *event)
{
    ...
    {
        initial.x = event->pos().x()/100.0;
        initial.y = event->pos().y()/100.0;
    }
    else if(event->button()==Qt::RightButton)
    {
        final.x = event->pos().x()/100.0;
        final.y = event->pos().y()/100.0;
    }
}

void Field::mouseReleaseEvent(QGraphicsSceneMouseEvent *event)
{
    auto dx = event->pos().x()/100.0 - initial.x;
    auto dy = event->pos().y()/100.0 - initial.y;
    if(event->button()==Qt::LeftButton)
    {
        initial.dx = dx;
        initial.dy = dy;
